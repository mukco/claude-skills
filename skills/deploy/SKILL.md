...
- **`denied: permission_denied: read_package` when CI pushes to ghcr.io** = the package was first created by a laptop push (PAT) and is not linked to the repo, so `GITHUB_TOKEN` has no rights; the OCI `image.source` label does NOT retro-link an existing package. Fix: delete the package (`gh auth refresh -s delete:packages`, then `gh api -X DELETE user/packages/container/<name>`) and let the next Actions push recreate it linked to the repo — or Package settings → Manage Actions access → add the repo with Write. New apps from the template deploy from CI first, so they never hit this.
